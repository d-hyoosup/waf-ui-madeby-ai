...
export const getStatusBadgeClass = (state: BackupStatus): string => { // status -> state
  switch (state) {
...
export const getStatusText = (state: BackupStatus): string => { // status -> state
  switch (state) {
...
export const getRollbackActionBadgeClass = (action: string): string => {
  switch (action) {
    ...
    case 'RESTORE': return 'badge-restoring';
    default: return 'badge-secondary';
  }
};